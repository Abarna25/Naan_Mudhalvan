...
export default function FacultyAnalyticsPage() {
  ...
  // Helper to extract action modules from missing areas string
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto relative">
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg p-6 space-y-5 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="font-bold text-slate-100 text-base">Assign Intervention Roadmap</h3>
              </div>
            ...
            <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1.5 text-xs">
              <div className="flex justify-between font-bold text-slate-200">
                ...
                <span className="text-rose-400">Eligibility Score: {selectedStudent.score}%</span>
              </div>
              <p className="text-slate-400">CGPA: {selectedStudent.cgpa} | Dept: Computer Science & Engineering</p>
              <div className="flex items-center space-x-1.5 pt-1">
                <span className="text-slate-500 font-semibold">Identified Gaps:</span>
                <span className="text-rose-300 font-medium">{selectedStudent.missingAreas}</span>
              </div>
            </div>

            <form onSubmit={handleAssignSubmit} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-400 mb-1 font-semibold">Target Job Role</label>
                <select
                  value={targetRole}
                  onChange={e => setTargetRole(e.target.value)}
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5 text-slate-200 focus:outline-none focus:border-indigo-500"
                >
              ...
              <div>
                <label className="block text-slate-400 mb-1 font-semibold">Intervention Timeline</label>
                  ...
                  {[2, 4, 6, 8].map(w => (
                    <button
                      ...
                      className={`py-2 rounded-xl font-bold border transition-all ${
                        timelineWeeks === w ? 'bg-indigo-600 text-white border-indigo-500 shadow-md shadow-indigo-600/30' : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700'
                      }`}
                    >
                      {w} Weeks
                    </button>
              ...
              <div>
                <label className="block text-slate-400 mb-2 font-semibold">Automated Action Modules (Based on Identified Gaps)</label>
                <div className="space-y-2">
                  {getActionModules(selectedStudent.missingAreas).map((mod, idx) => (
                    <div key={idx} className="flex items-center space-x-2.5 p-2.5 rounded-xl bg-slate-950 border border-indigo-900/40 text-indigo-200">
              ...
              <div>
                <label className="block text-slate-400 mb-1 font-semibold">Faculty Instructions / Guidance Notes</label>
                <textarea
                  rows={3}
                  value={facultyNotes}
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-slate-300 font-semibold hover:bg-slate-700 transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold transition-all shadow-lg shadow-indigo-600/30 flex items-center justify-center space-x-2 disabled:opacity-60"
                >
                  {isSubmitting ? (
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Department-wide student progress, top performers, and intervention alerts.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="glass-card p-5 rounded-2xl border border-slate-800">
          <p className="text-xs text-slate-400 font-semibold uppercase">Dept Students</p>
          <h3 className="text-3xl font-black text-white mt-1">124</h3>
          <p className="text-[11px] text-emerald-400 mt-1 flex items-center"><ArrowUp className="w-3 h-3 mr-1" />+4 this semester</p>
        </div>
        <div className="glass-card p-5 rounded-2xl border border-slate-800">
          <p className="text-xs text-slate-400 font-semibold uppercase">Avg Eligibility</p>
          <h3 className="text-3xl font-black text-emerald-400 mt-1">84.5%</h3>
          <p className="text-[11px] text-emerald-400 mt-1 flex items-center"><ArrowUp className="w-3 h-3 mr-1" />+8% vs Jan baseline</p>
        </div>
        <div className="glass-card p-5 rounded-2xl border border-slate-800">
          <p className="text-xs text-slate-400 font-semibold uppercase">Tier-1 Ready</p>
          <h3 className="text-3xl font-black text-blue-400 mt-1">42</h3>
          <p className="text-[11px] text-blue-400 mt-1">Score ≥ 85%</p>
        </div>
        <div className="glass-card p-5 rounded-2xl border border-slate-800">
          <p className="text-xs text-slate-400 font-semibold uppercase">Needs Intervention</p>
          <h3 className="text-3xl font-black text-rose-400 mt-1">8</h3>
          <p className="text-[11px] text-rose-400 mt-1 flex items-center"><AlertTriangle className="w-3 h-3 mr-1" />Score below 70%</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 rounded-2xl border border-slate-800 space-y-3">
          <h3 className="font-bold text-slate-100">Class Eligibility Score Trend (Jan–Jun 2026)</h3>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trendData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                <XAxis dataKey="month" stroke="#94a3b8" tick={{ fontSize: 11 }} />
                <YAxis stroke="#94a3b8" domain={[60, 90]} tick={{ fontSize: 11 }} />
                <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155' }} />
                <Line type="monotone" dataKey="avgScore" stroke="#3b82f6" strokeWidth={2} dot={{ r: 4, fill: '#3b82f6' }} name="Avg Score %" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        {/* Skill Radar */}
        <div className="glass-card p-6 rounded-2xl border border-slate-800 space-y-3">
          <h3 className="font-bold text-slate-100">Department Skill Readiness Spectrum</h3>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="glass-card p-6 rounded-2xl border border-slate-800 space-y-4">
        <h3 className="font-bold text-slate-100">Primary Skill Distribution Across CSE Department</h3>
        <div className="h-48">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={skillDistribution} layout="vertical">
              <XAxis type="number" stroke="#94a3b8" tick={{ fontSize: 11 }} />
              <YAxis dataKey="skill" type="category" stroke="#94a3b8" tick={{ fontSize: 11 }} width={120} />
              <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155' }} />
      ...
      {/* Intervention Needed */}
      <div className="glass-card p-6 rounded-2xl border border-rose-900/40 bg-rose-950/10 space-y-4">
        <h3 className="font-bold text-rose-300 flex items-center space-x-2">
        ...
        <div className="space-y-3">
          {interventionNeeded.map((s, idx) => {
            ...
            return (
              <div key={idx} className="p-4 rounded-xl bg-slate-900/60 border border-rose-900/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                <div className="space-y-1">
                  ...
                  <p className="text-slate-400">CGPA: {s.cgpa} | Eligibility: <span className="text-rose-400 font-bold">{s.score}%</span></p>
                  <p className="text-rose-400/70">Missing Gaps: {s.missingAreas}</p>
                </div>
                <button
                  onClick={() => handleOpenAssignModal(s)}
                  className={`px-3.5 py-2 text-[11px] font-semibold rounded-xl border transition-all flex items-center justify-center space-x-1.5 shrink-0 ${
                    assigned
